...
/** SessionShortValueCollect. */
@FunctionHint(output = @DataTypeHint("ROW<collect_result ARRAY<SMALLINT>>"))
public class SessionShortValueCollect extends TableFunction<Row> {
  ...
  public void eval(byte[] valueSeries) throws Exception {
    ...
    row.setField(0, result.toArray(new Short[] {}));
    collect(row);
  }
}
