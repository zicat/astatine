...
public class Sort {
  ...
  /**
   * Stablized quick sort on an index array. This is a normal quick sort that uses the original
   * index as a secondary key. Since we are really just sorting an index array we can do this nearly
   * for free.
   *
  ...
  private static void stableQuickSort(int[] order, double[] values, int start, int end, int limit) {
    ...
    while (end - start > limit) {
      ...
      if (from == low) {
        // ran out of things to copy.  This means that the the last destination is the
        // boundary
        low = to + 1;
      } else {
  ...
  public static void checkPartition(
      int[] order, double[] values, double pivotValue, int start, int low, int high, int end) {
  ...
  /**
   * Limited range insertion sort with primary key stabilized by the use of the original position to
   * break ties. We assume that no element has to move more than limit steps because quick sort has
   * done its thing.
   *
