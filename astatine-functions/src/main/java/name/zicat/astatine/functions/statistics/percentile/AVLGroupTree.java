...
public class AVLGroupTree extends AbstractCollection<Centroid> implements Serializable {
  ...
  public int[] aggregatedCounts;
  public IntAVLTree tree;

  public AVLGroupTree(final boolean record) {
    tree =
        new IntAVLTree() {
  ...
  public List<Double> data(int node) {
    return dataList == null ? null : dataList[node];
  }
  ...
  /**
   * Compute the number of elements and sum of counts for every entry that is strictly before <code>
   * node</code>.
   */
  @SuppressWarnings("WeakerAccess")
