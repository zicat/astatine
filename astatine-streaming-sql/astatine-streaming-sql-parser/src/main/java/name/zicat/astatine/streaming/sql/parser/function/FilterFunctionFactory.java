...
public interface FilterFunctionFactory<T> extends OneTransformFunctionFactory<DataStream<T>, T, T> {
   ...
   */
  FilterFunction<T> createFilter(TransformContext context);
   ...
   * @return returns.
   */
  TypeInformation<T> returns();

  @Override
  default DataStream<T> transform(TransformContext context, DataStream<T> stream) {
    var result = stream.filter(createFilter(context)).name(identity());
