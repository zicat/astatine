...
operators
    : (operatorWithKeyValues)*
    | EOF
    ;

operatorWithKeyValues
    : operatorType (multipartIdentifier)? (keyValues)?
    | operatorTypeWithOutKeyValues
    ;
...
// throw exception if not register java spi 
unknownOperatorType
    : identifier
    ;
...
nonReserved
    : ADD | ALTER | AS | ASC | BRANCH | BY | COMPACT | STREAM | INSERT | INTO | VIEW | FROM | CLEAN | CREATE | DAYS | DESC | DROP | EXISTS | FIELD | FIRST | HOURS | IF | LAST | NOT | NULLS | OF | OR | ORDERED | PARTITION | TABLE | WRITE
