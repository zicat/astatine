...
public class EventtimeOrderEmitterFunctionFactoryTest extends TransformFactoryTestBase {
  ...
  public void test() throws Exception {
    ...
    row1.setField(1, TimestampData.fromEpochMillis(ts + 2000));
    row1.setField(2, 1);
    final var row2 = new GenericRowData(3);
    row2.setField(0, StringData.fromString("name2"));
    row2.setField(1, TimestampData.fromEpochMillis(ts + 1000));
    row2.setField(2, 1);
    ...
    final var source =
        env.fromCollection(Arrays.<RowData>asList(row1, row2, row3))
            .assignTimestampsAndWatermarks(TimestampWatermarkGenerator.create(1, -5000))
            .returns(
                InternalTypeInfo.of(
                    new RowType(
                        Arrays.asList(
    ...
    TransformFactoryTestBase.execAndAssert(
        ...
        data -> {
          ...
          for (int i = 0; i < data.size(); i++) {
            final var timestamp = ((RowData) data.get(i)).getTimestamp(1, 3);
            Assert.assertEquals(ts + 1000L * (i + 1), timestamp.getMillisecond());
          }
        });
  }
}
