...
public class TemporalJoinConnectionFunction<T>
                ...
                getRuntimeContext()
                        .getMapState(
                                new MapStateDescriptor<>(
                                        LEFT_STATE_NAME,
                                        Types.LONG,
                                        new ListTypeInfo<>(leftReturnRowTypeInfo)));
        rightState =
                getRuntimeContext()
                        .getMapState(
                                new MapStateDescriptor<>(
                                        RIGHT_STATE_NAME, Types.LONG, rightReturnRowTypeInfo));
        cleanupTimeState =
                getRuntimeContext()
                        .getState(new ValueStateDescriptor<>(CLEANUP_STATE_NAME, Types.LONG));
        registeredTimer =
                getRuntimeContext()
                        .getState(
    ...
    private void registerSmallestTimer(long timestamp, TimerService timerService)
            throws IOException {
    ...
    private void cleanupExpiredVersionInState(
            long currentWatermark, List<Map.Entry<Long, RowData>> rightRowsSorted)
            throws Exception {
        int i = 0;
