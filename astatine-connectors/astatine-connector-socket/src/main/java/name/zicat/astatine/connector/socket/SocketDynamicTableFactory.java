...
public final class SocketDynamicTableFactory implements DynamicTableSourceFactory {
  ...
  public DynamicTableSource createDynamicTableSource(Context context) {
    ...
    final var options = helper.getOptions();
    final var hostname = options.get(HOSTNAME);
    final var port = options.get(PORT);
    final var byteDelimiter = (byte) (int) options.get(BYTE_DELIMITER);

    // derive the produced data type (excluding computed columns) from the catalog table
    final var producedDataType =
