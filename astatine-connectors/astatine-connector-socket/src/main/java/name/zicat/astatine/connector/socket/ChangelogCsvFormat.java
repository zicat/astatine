...
package name.zicat.astatine.connector.socket;
...
public final class ChangelogCsvFormat implements DecodingFormat<DeserializationSchema<RowData>> {
    ...
    public DeserializationSchema<RowData> createRuntimeDecoder(
        ...
        final var producedTypeInfo = context.<RowData>createTypeInformation(producedDataType);

        // most of the code in DeserializationSchema will not work on internal data structures
        // create a converter for conversion at the end
        final var converter = context.createDataStructureConverter(producedDataType);

        // use logical types during runtime for parsing
        final var parsingTypes = producedDataType.getLogicalType().getChildren();
