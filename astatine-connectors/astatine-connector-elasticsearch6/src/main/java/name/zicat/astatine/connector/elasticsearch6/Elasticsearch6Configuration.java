...
public class Elasticsearch6Configuration extends ElasticsearchConfiguration {
  Elasticsearch6Configuration(ReadableConfig config, ClassLoader classLoader) {
    super(config, classLoader);
  }

  public List<HttpHost> getHosts() {
    return config.get(ElasticsearchConnectorOptions.HOSTS_OPTION).stream()
  ...
  private static HttpHost validateAndParseHostsString(String host) {
    try {
      final var httpHost = HttpHost.create(host);
